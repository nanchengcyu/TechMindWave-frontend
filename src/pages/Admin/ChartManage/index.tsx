//管理员操作表格
import { deleteChartUsingPOST, listChartByPageUsingPOST } from '@/services/nanchengyubi/ChartController';
import {Link, useModel} from '@@/exports';
import { ExclamationCircleOutlined } from '@ant-design/icons';
import {Avatar, Button, Card,  Divider, List, message, Modal, Result} from 'antd';
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div>
                  <Link to={`/ViewChartData/${item.id}`}>
                    <Button style={{ backgroundColor: '#007BFF', color: 'white' }}>查看图表数据</Button>
                  </Link>
              ...
              {item.chartStatus ==='succeed' && (
                <>
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ background: 'black', padding: '5px 10px', borderRadius: '4px', display: 'inline-block' }}>
                      <Divider style={{ fontSize: '18px', fontWeight: 'bold', color: 'white', margin: 0 }}>{'图表标题：' + item.goal}</Divider>
                    </div>
                  </div>
                  <List.Item.Meta
                  ...
                  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ background: 'black', padding: '5px 10px', borderRadius: '4px', display: 'inline-block' }}>
                      <Divider style={{ fontSize: '18px', fontWeight: 'bold', color: 'white', margin: 0 }}>智能分析结果</Divider>
                    </div>
                  </div>

                  <div style={{ whiteSpace: 'pre-wrap', overflow: 'auto' }}>
                    <p style={{ fontWeight: 'bold', color: 'black', textAlign: "left" }}>
                      {item.genResult}
                    </p>
                  </div>
                </>
